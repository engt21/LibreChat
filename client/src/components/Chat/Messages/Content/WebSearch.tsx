...
export default function WebSearch({
  ...
  const webSearchAction = useRecoilValue(store.latestWebSearchAction);
  ...
  return (
    <div className="my-1 flex flex-col gap-1">
      ...
      {webSearchAction?.queries && webSearchAction.queries.length > 0 && (
        <div className="ml-6.5 flex flex-col gap-0.5 text-xs text-text-tertiary">
          {webSearchAction.queries.map((q, i) => (
      ...
      {webSearchAction?.sources && webSearchAction.sources.length > 0 && (
        <div className="ml-6.5 flex flex-col gap-0.5 text-xs text-text-tertiary">
          {webSearchAction.sources.slice(0, 5).map((s, i) => (
            <a
              key={i}
              href={s.url}
              target="_blank"
