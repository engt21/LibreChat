...
describe('MFA enforcement policy', () => {
  ...
  test('requires enrollment for local password users', () => {
    expect(requiresMFAEnrollment({ provider: 'local', password: 'hash', twoFactorEnabled: false })).toBe(true);
  });

  test('does not double-enforce enrolled users', () => {
    expect(requiresMFAEnrollment({ provider: 'local', password: 'hash', twoFactorEnabled: true })).toBe(false);
  });

  test('leaves federated provider MFA to the identity provider', () => {
    expect(requiresMFAEnrollment({ provider: 'openid', twoFactorEnabled: false })).toBe(false);
  });

  test('honors a future enforcement date', () => {
    process.env.MFA_ENFORCE_AFTER = '2099-01-01T00:00:00Z';
    expect(requiresMFAEnrollment({ provider: 'local', password: 'hash', twoFactorEnabled: false })).toBe(false);
  });
});
