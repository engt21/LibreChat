...
- Work serially for heavy validation or runtime exports/imports; host memory is constrained and validation concurrency stays at `1`. For code-only changes, use `docker cp` + `docker restart` instead of full Docker image rebuilds. Only rebuild images when Dockerfile, package.json, or package-lock.json change.
