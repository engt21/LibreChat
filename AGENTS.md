...
`local-services/deploy-built-client-dist.sh` strictly rejects missing/stale/tampered build manifests and cache-busted asset URLs, snapshots the prior deployed `client/dist`, stages and swaps the entire built tree, restarts the API process to clear cached `index.html`, verifies HTTP entry assets, automatically restores the rollback tree on health failure, and snapshots a verified stable promotion into `librechat-local:runtime-current` so later container recreation retains the frontend without a full rebuild.
