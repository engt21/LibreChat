...
- keep `client/scripts/post-build.cjs` generating `.librechat-client-dist-manifest.json` and keep `local-services/deploy-built-client-dist.sh` as the only supported frontend hot-promotion path; it must reject stale/tampered/URL-rewritten bundles, snapshot and atomically swap the complete `client/dist`, restart to clear cached HTML, rollback on failed health verification, and persist a verified stable promotion to `librechat-local:runtime-current` so recreation cannot restore an older UI
